...
class OSRSbowstring(OSRSBot):
    def __init__(self):
        self.clay = None
        self.options_set = True
        bot_title = "string yew longs"
        description = (
            "NA"
        )
    ...
    def main_loop(self):
        ...
        yewlogs = 'Yew_longbow_(u)'
        string = 'Bow_string'
        while time.time() - start_time < end_time:
            if api_m.get_is_player_idle(2):
                if api_m.get_if_item_in_inv([ids.YEW_LONGBOW_U,ids.BOW_STRING]):
                ...
                if api_m.get_is_inv_full:
                    ...
                    time.sleep(0.6)
                    self.deposit_glass()
                    self.withdraw_from_bank([f'{yewlogs}_bank'],False)
                    self.withdraw_from_bank([f'{string}_bank'])
    ...
    def __craft_glass(self,api_m:MorgHTTPSocket):
        pipe = api_m.get_inv_item_indices(ids.BOW_STRING)
        if not pipe:
            self.__logout("no knife located")
        self.mouse.move_to(self.win.inventory_slots[pipe[0]].random_point(),mouseSpeed='fast')
        self.mouse.click()
        glass = api_m.get_inv_item_indices(ids.YEW_LONGBOW_U)
        if  glass: 
            self.mouse.move_to(self.win.inventory_slots[glass[-1]].random_point(),mouseSpeed='fast')
        self.mouse.click()
        time.sleep(1.2)
